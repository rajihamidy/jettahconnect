...
if (isset($_POST['email']) && isset($_POST['recovery_code']) && isset($_POST['new_password']) && isset($_POST['cnew_password'])) {
    ...
    $sql = "SELECT recovery_code, recovery_expiration FROM user_info WHERE email = '$email'";
    ...
    if ($row) {
        ...
        if ($recovery_code === $stored_recovery_code && strtotime($recovery_expiration) > time()) {
            $update_sql = "UPDATE user_info SET password = '$hashed_password', recovery_code = NULL, recovery_expiration = NULL WHERE email = '$email'";
